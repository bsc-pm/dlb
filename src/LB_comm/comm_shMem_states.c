...
void LoadCommConfig(int num_procs, int meId, int nodeId){
	...
	if (me==0){
		sm_size= sizeof(sharedData) + (sizeof(ProcMetrics)*size_queue) + (sizeof(sem_t)*procs)+ (sizeof(int)*procs)+ (sizeof(sem_t)*size_queue);
...
/*		if (sem_init(&(shData->queue), 1, size_queue)<0){
			perror("sem_init lock_data");
			exit(1);
		}*/

		//The queue of messages for the master
		msgs = (ProcMetrics*)((char*)shData + sizeof(sharedData)); 
		for (i=0; i<size_queue; i++){
			msgs[i].action=-1;
		}

		//The array of sempahores to warn the slaves of a change in the number of threads
		msg4slave =  (sem_t*)((char*)msgs + (sizeof(ProcMetrics)*size_queue)); 
		for (i=0; i<procs; i++){
		...
		//The semaphores to have exclusive acces to the data
		lock_data = (sem_t*)((char*)threads + (sizeof(int)*procs));
		for (i=0; i<procs; i++){
			if (sem_init(&(lock_data[i]), 1, 1)<0){
				perror("sem_init lock_data");
				exit(1);
			}
		}
		...
		sm_size= sizeof(sharedData) + (sizeof(ProcMetrics)*size_queue) + (sizeof(sem_t)*procs)+ (sizeof(int)*procs);
		//         the struct         the queue of messages       the semaphores         the threads         
...
void printQueue(){
	int i; 
	fprintf(stderr,"%d:%d - Queue: ", node, me);
	for (i=0; i<size_queue; i++){
		fprintf(stderr,"[%d,%d]", i, msgs[i].action);
	}
...
int GetFromAnySlave(char *info,int size){
    ...
    printQueue();
#endif
	while((slave!=last_sender)&&(msgs[slave].action<0)) slave=((slave+1)%size_queue);

	if(msgs[slave].action>=0){
		if (sem_wait(&(lock_data[slave]))<0)
			perror("sem_wait lock_data master");

		memcpy(info, &(msgs[slave]), size);
 		
		if(msgs[slave].action>=0) msgs[slave].action=-2;

		if(sem_post(&(lock_data[slave]))<0)
			perror("sem_wait lock_data master");
...
//	sem_wait(&(shData->queue));
	...
	if (msgs[me].action>=0){
#ifdef debugSharedMem 
    	fprintf(stderr,"%d:%d Discarding previous messsage\n", node, me);
#endif
		msgs[me].action=-1;
	}else{
		memcpy(&(msgs[me]), info, size);
				
		if (sem_post(&(shData->msg4master))<0){
			perror("Post to master");
		}
	}
    sem_post(&(lock_data[me]));
