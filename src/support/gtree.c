...
g_tree_ref (GTree *tree)
{
  ...
  __sync_fetch_and_add(&tree->ref_count, 1);
...
g_tree_unref (GTree *tree)
{
  ...
  if (__sync_fetch_and_sub ((&tree->ref_count), 1) == 1)
    {
  ...
  while (1)
    {
      ...
      ensure(!bparent || bparent->left == node || bparent->right == node,
              "g_tree_insert_internal() error");
                        ...
                        gboolean       steal)
{
  ...
  ensure(!parent || parent->left == node || parent->right == node,
          "g_tree_remove_internal() error");
  ...
  if (balance)
    while (1)
      {
        GTreeNode *bparent = path[--idx];
        ensure(!bparent || bparent->left == balance || bparent->right == balance,
                "g_tree_remove_internal() error");
                 ...
                 gpointer       user_data)
{
  ...
  switch (traverse_type)
    ...
    case G_LEVEL_ORDER:
      warning("g_tree_traverse(): traverse type G_LEVEL_ORDER isn't implemented.");
      break;
    }
}
